...
/*
 * Initialize filesystem. Read in file system metadatma and initialize
...
static void* vfs_mount(struct fuse_conn_info *conn) {
    ...
    cache_initialize(vcBlock->total_size);	
...
static int vfs_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
	...
	dread_cache(0, vcBlock);char* data_cache;
char* tags_cache;
	dirent* dirEntry = (dirent*)calloc(1, sizeof(dirent));
	if (!dirEntry) {
		return -1;
...
static int vfs_write(const char *path, const char *buf, size_t size,
                     off_t offset, struct fuse_file_info *fi)
{
  /* 3600: NOTE THAT IF THE OFFSET+SIZE GOES Ochar* data_cache;
char* tags_cache;FF THE END OF THE FILE, YOU
